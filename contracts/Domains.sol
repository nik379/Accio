...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) payable ERC721("Accio Name Service", "Floooo") {
    tld = _tld;
  ...
  // Function to give the price of a domain based on length
  function price(string calldata name) public pure returns(uint) {
    uint len = StringUtils.strlen(name);
    require(len > 0 , "Wow you are creative but we can't register this!!");
    if (len <= 3) {
      return 5 * 10**16; 
    } else if (len == 4) {
      return 3 * 10**16; 
  ...
  function getAddress(string calldata name) public view returns (address) {
   ...
   function setRecord(string calldata name, string calldata record) public {
      ...
      require(domains[name] == msg.sender , "You don't have the rights to change this domain records.");
      records[name] = record;
  }
